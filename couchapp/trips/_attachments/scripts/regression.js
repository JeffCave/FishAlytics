...
;(function() {
	...
	var methods = {
		...
		power: function(data) {
			var sum = {lnX:0, prod:0, lnY:0, sqr:0, count: 0};
			
			data.forEach(function(point){
				// first make sure nothing really bone headed was passed in
				var 
				...
				// then calculate the natural log
				x = Math.log(x);
				y = Math.log(y);
			...
			// now calculte the regression
			var B = (sum.count * sum.prod - sum.lnY * sum.lnX) / (sum.count * sum.sqr - sum.lnX * sum.lnX);
			var A = Math.exp((sum.lnY - B * sum.lnX) / sum.count);
			
			// and prep some other results
			var $string = 'y = ' + Math.round(A*100.0)/100 + 'x^' + Math.round(B*100.0)/100;
			var $getY = "(function(x){ return ("+A+" * Math.pow(x,"+B+")); })";
			var $getX = "(function(y){ return Math.pow((y/"+A+"),"+(1.0/B)+"); })";
