namespace Vius.Web
{
	...
	public static class ParamSerializer
	{
		...
		{
			var paramString = Parametrize(JObject.FromObject(obj));
			return paramString.TrimEnd(new[] { '&' });
		}

		public static object Deserialize (NameValueCollection input)
		{
			return Deserialize(ConvertNameValueCollection(input));
		}

		public static T Deserialize<T> (NameValueCollection input)
		{
			return Deserialize<T>(ConvertNameValueCollection(input));
		}
		...
		{
			return Deparametrize(input).ToObject<T>();
		}
		...
		private static string Parametrize (object obj, string value = "")
		{
			...
			if (obj is JObject) {
				...
				for (int i = 0; i < arr.Count; i++) {
					...
					if (item is JArray || item is JObject) {
						returnVal += Parametrize(item, string.Format("{0}[{1}]", value, i));
					}
					else {
						returnVal += Parametrize(item, string.Format("{0}[]", value));
					}
				}
			}
			else {
		...
		private static string ConvertNameValueCollection (NameValueCollection input)
		{
